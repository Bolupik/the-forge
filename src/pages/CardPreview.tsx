...
const CardPreview = () => {
  ...
  return (
    <div className="min-h-screen p-6 sm:p-10 flex flex-col items-center gap-8" style={{ background: 'var(--cf-bg, #07070d)' }}>
      <h1 className="font-display text-2xl sm:text-3xl tracking-wider" style={{ color: 'var(--cf-gold, #e0483a)' }}>
      ...
      <div className="w-full max-w-md space-y-6 p-5 rounded-lg"
           style={{ background: 'rgba(13,13,26,0.7)', border: '1px solid rgba(224,72,58,0.25)' }}>
        {/* Rarity tabs */}
        <div>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {RARITIES.map(r => (
              <button
                ...
                style={{
                  background: rarity === r ? 'rgba(224,72,58,0.18)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${rarity === r ? 'rgba(224,72,58,0.55)' : 'rgba(255,255,255,0.08)'}`,
                  color: rarity === r ? 'var(--cf-gold, #e0483a)' : 'var(--cf-muted2, #aaa)',
                }}
              >
                {r}
              </button>
        ...
        <div>
          <div className="flex items-baseline justify-between mb-2">
            ...
            <span className="font-mono text-[0.65rem]" style={{ color: 'var(--cf-gold, #e0483a)' }}>{intensity.toFixed(2)}×</span>
          </div>
          <Slider
            min={0}
            max={2.5}
          ...
          <div className="grid grid-cols-4 gap-1.5 mt-3">
            {PRESETS.map(p => (
              <button
                ...
                style={{
                  background: Math.abs(intensity - p.value) < 0.01 ? 'rgba(224,72,58,0.18)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${Math.abs(intensity - p.value) < 0.01 ? 'rgba(224,72,58,0.55)' : 'rgba(255,255,255,0.08)'}`,
