...
(define-constant err-empty-pack          (err u111))
...
;; ------------------------------------------------------------
;; BATCH MINTING -- one payment (mint-price) for a whole pack
;; ------------------------------------------------------------
;; Card input shape shared by the fold helper and the public entrypoint.
(define-private (mint-one
    (card {
      name: (string-ascii 64),
      rarity: (string-ascii 16),
      card-type: (string-ascii 32),
  ...
  (match prior
    ok-count
      (let ((new-id (+ (var-get last-token-id) u1)))
        (asserts! (<= new-id (var-get max-supply)) err-max-supply)
        (asserts! (is-valid-rarity (get rarity card)) err-invalid-rarity)
        (try! (nft-mint? cardforge-card new-id tx-sender))
        (map-set card-metadata new-id {
          name: (get name card),
          rarity: (get rarity card),
          card-type: (get card-type card),
          power: (get power card),
          defense: (get defense card),
          image-uri: (get image-uri card),
          token-uri: (get token-uri card),
          creator: tx-sender
        })
        (var-set last-token-id new-id)
        (print { event: "card-minted", token-id: new-id, owner: tx-sender, rarity: (get rarity card) })
        (ok (+ ok-count u1)))
    err-code (err err-code)))

;; Mint every card in the pack but charge `mint-price` exactly ONCE.
;; Token ids are sequential: first-token-id .. (first-token-id + count - 1).
(define-public (mint-pack
    (cards (list 10 {
      name: (string-ascii 64),
      rarity: (string-ascii 16),
      card-type: (string-ascii 32),
  ...
  (begin
    (asserts! (var-get mint-enabled) err-mint-disabled)
    (asserts! (> (len cards) u0) err-empty-pack)
    ;; Single payment for the whole pack.
    (try! (stx-transfer? (var-get mint-price) tx-sender (var-get treasury)))
    (let (
          (first-id (+ (var-get last-token-id) u1))
          (count (try! (fold mint-one cards (ok u0))))
         )
      (print { event: "pack-minted", count: count, first-token-id: first-id, owner: tx-sender })
      (ok { first-token-id: first-id, count: count })
    )
  )
)
